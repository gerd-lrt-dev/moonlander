...
void simcontrol::runAutopilot(const SpacecraftState& currentSpacecraftstate, const int &engineNr, const double& dt)
{
    ...
    if(currentSpacecraftstate == SpacecraftState::Operational)
    {
        ...
        double autoThrust = autopilot_->setAutoThrustInNewton(controller_.get(), spacecraftConfig_.engines_[0].maxThrust, landerSpacecraft->ENU_getVelocity().z(), landerSpacecraft->ENU_getPosition().z() - config_.radiusMoon, dt, spacecraftConfig_.emptyMass + spacecraftConfig_.fuelM, config_.moonGravity);
