...
void LandingView::setPositionENU(const Eigen::Vector3d& pos)
{
    ...
    if (trajectoryEN.isEmpty() ||
        QLineF(trajectoryEN.last(), enPoint).length() > 0.5)
    {
...
void LandingView::setMainEngineDirectionENU(
    const Eigen::Vector3d& direction)
{
    mainEngineDirectionENU = direction;
    update();
}
...
void LandingView::drawPanelFrame(
    QPainter& p,
    const QRect& r,
    const QString& title)
{
...
QPointF LandingView::mapSideView(
    const QRect& r,
    double east,
    double up,
    double maxEastAbs,
...
QPointF LandingView::mapTopView(
    const QRect& r,
    double east,
    double north,
    double maxEastAbs,
...
void LandingView::drawLanderSide(
    QPainter& p,
    ...
    double attitudeDeg)
{
    ...
    /*
     * Positive ENU East-Up inclination is mapped to the Qt
     * screen coordinate system, whose Y axis points downward.
     */
    p.rotate(attitudeDeg);
...
void LandingView::drawLanderTop(
    QPainter& p,
    const QPointF& center,
    double yawDeg_)
{
    ...
    const QRect& r)
{
    ...
    const double maxEastAbs =
        qMax(
            50.0,
            qAbs(positionENU.x()) + 10.0
            );
    ...
    /*
     * Project the main-engine direction onto the E-UP plane.
     *
     * atan2(East, Up) yields:
     *   0 deg   -> engine/thrust axis aligned with +Up
     *  +90 deg  -> axis aligned with +East
     *  -90 deg  -> axis aligned with -East
     */
    const double sideAttitudeDeg =
        qRadiansToDegrees(
            std::atan2(
                mainEngineDirectionENU.x(),
                mainEngineDirectionENU.z()
                )
            );
    ...
    /*
     * Main-engine thrust direction projected into the E-UP plane.
     *
     * The Y component of mainEngineDirectionENU is intentionally omitted
     * because the side view only represents East and Up.
     */
...
void LandingView::drawStatusBox(
    QPainter& p,
    const QRect& r)
{
    drawPanelFrame(
